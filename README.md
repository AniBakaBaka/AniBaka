...
**Baka** 是使用 Flutter 开发的支持多番剧源和弹幕的看番软件。部分兼容kazumi规则，也支持animeko的css规则（这不代表一定安装及可播放），支持规则导入与规则分享，支持部分外来源播放，支持基于 Anime4K 的实时超分辨率，支持弹幕、投屏、字幕管理等超多功能。
...
- **自定义源**: 支持加载自定义 css/xpath 适配器，也就是kazumi/animeko编写好后的规则可以直接导入到 Baka 中。
- **聚合搜索**: 一搜索，全网有。不再需要在不同网站间反复横跳。
- **Kazumi 远程规则**: 支持从 GitHub 远程仓库拉取 Kazumi 规则列表，内置中国大陆镜像加速，

>**注意**由于与kazumi/animeko获取视频链接的实现方式不同，有些kazumi/animeko可以播放，但baka不能播放，需要进行微调，不一定可以直接安装就可以使用源。
- **验证码自动处理**: 部分源支持全自动验证码通过。
- **自定义源管理**: 提供完整的源管理界面，支持新建、编辑、启用/禁用、导出和导入源配置。支持 `baka://` 链接、`kazumi://` 链接和原始 JSON 格式导入
### 📺 播放器
- **mpv 内核**: 基于 `media_kit` (libmpv)
- **Anime4K 实时增强**: 全端内置 Anime4K 算法，一键将 720p/1080p 提升至 4K 画质
...
- **番剧详情页**: 查看番剧的完整信息，包括 Infobox（制作人员/声优）、收藏状态分布（想看/在看/看过/搁置/抛弃）。
...
PS：建议某些开发者这辈子可不要用AI来编写界面，您的界面确实手搓好看，我到处抄抄就可以实现这么多源，而且还能解决某些源需要验证码的问题，好奇怪啊，怎么光抄就可以解决这个问题啊，据我所知目前AI还是喜欢用md3风格的。
