...
- **自定义源**: 支持加载自定义 css/xpath 适配器，也就是kazumi/animeko编写好后的规则可以直接导入到 Baka 中，由于与kazumi/animeko获取视频链接的实现方式不同，有些kazumi/animeko可以播放，但baka不能播放，需要进行微调，不一定可以直接安装就可以使用源。
